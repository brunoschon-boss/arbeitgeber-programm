...
phases.forEach((p, i) => { if (i <= activeIndex) p.classList.add('active'); else p.classList.remove('active'); });
...
items.forEach(item => {
const key = item.querySelector('.dky');
if (!key) return;
...
trigger.addEventListener('click', () => {
const wasOpen = item.classList.contains('open');
obsItems.forEach(i => i.classList.remove('open'));
if (!wasOpen) {
